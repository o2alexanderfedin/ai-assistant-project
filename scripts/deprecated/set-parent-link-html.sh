...
# This script sets the custom "Parent Link" field using HTML formatting
...
PARENT_LINK="<a href=\"$PARENT_URL\">#$PARENT_NUM</a> - $PARENT_TITLE"
...
echo "🔄 Setting parent link field with HTML format..."
...
if [[ "$RESULT" == *"projectV2Item"* ]]; then
  echo "✅ Successfully set parent link for issue #$CHILD_NUM to parent #$PARENT_NUM using HTML format"
else
...
echo ""
echo "💡 The Parent Link field now contains an HTML link to issue #$PARENT_NUM"
