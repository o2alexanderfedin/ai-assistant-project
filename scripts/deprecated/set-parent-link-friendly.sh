...
# This script sets the custom "Parent Link" field using a more friendly format
...
# ----------- CREATE PARENT LINK ----------- #
# Using a more friendly format with emoji
...
# Format: "Epic #8: Epic Prioritization..." 
# Or with emoji: "🔄 Epic #8: Core Agent..."
PARENT_LINK="$EMOJI""Epic #$PARENT_NUM: ${PARENT_TITLE#* }"
...
echo "🔄 Setting parent link field with friendly format..."
...
if [[ "$RESULT" == *"projectV2Item"* ]]; then
  echo "✅ Successfully set parent link for issue #$CHILD_NUM to parent #$PARENT_NUM using friendly format"
else
...
echo ""
echo "💡 The Parent Link field now contains a user-friendly reference to epic #$PARENT_NUM"
