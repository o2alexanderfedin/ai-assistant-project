...
# Created: $(date +%Y-%m-%d)
# Purpose: Set up custom fields and add epics and user stories to a GitHub Project
...
if [ -z "$1" ]; then
  ...
  echo "Example: $0 2"
  echo ""
  echo "You can create a new project with: ./scripts/create-github-project.sh"
  exit 1
fi

PROJECT_NUM=$1
echo "🚀 Setting up GitHub Project #$PROJECT_NUM for AI Assistant..."

# Step 1: Get project ID from number
echo "📋 Getting project ID..."
PROJECT_JSON=$(gh projects view $PROJECT_NUM --user '@me' --format json)
PROJECT_ID=$(echo $PROJECT_JSON | jq -r '.id')

echo "✅ Found project with ID: $PROJECT_NUM --user '@me'"

# Step 2: Create custom fields
echo "🏷️ Creating custom fields..."

# Type field (Epic, User Story, Task)
TYPE_FIELD_ID=$(gh projects field-create $PROJECT_NUM --user '@me' --name "Type" --data-type "SINGLE_SELECT" \
  --single-select-options "Epic,User Story,Task" --format json | jq -r '.id')
echo "  ✓ Created Type field"

# Priority field
PRIORITY_FIELD_ID=$(gh projects field-create $PROJECT_NUM --user '@me' --name "Priority" --data-type "SINGLE_SELECT" \
  --single-select-options "High,Medium,Low" --format json | jq -r '.id')
echo "  ✓ Created Priority field"

# Story Points field
POINTS_FIELD_ID=$(gh projects field-create $PROJECT_NUM --user '@me' --name "Story Points" --data-type "NUMBER" \
  --format json | jq -r '.id')
echo "  ✓ Created Story Points field"

# Component field
COMPONENT_FIELD_ID=$(gh projects field-create $PROJECT_NUM --user '@me' --name "Component" --data-type "SINGLE_SELECT" \
...
EPIC_LINK_FIELD_ID=$(gh projects field-create $PROJECT_NUM --user '@me' --name "Epic" --data-type "SINGLE_SELECT" \
  --format json | jq -r '.id')
...
for i in $(seq 0 $(($EPIC_COUNT-1))); do
  ...
  gh projects item-edit $PROJECT_NUM --user '@me' --id $ITEM_ID --field-id $TYPE_FIELD_ID \
    --single-select-option-id "Epic" >/dev/null
  
  # Add this epic to the Epic Link options for user stories
  gh projects field-edit $PROJECT_NUM --user '@me' --field-id $EPIC_LINK_FIELD_ID \
  ...
  if [ ! -z "$COMPONENT" ]; then
    gh projects item-edit $PROJECT_NUM --user '@me' --id $ITEM_ID --field-id $COMPONENT_FIELD_ID \
  ...
  if [ ! -z "$PRIORITY" ]; then
    gh projects item-edit $PROJECT_NUM --user '@me' --id $ITEM_ID --field-id $PRIORITY_FIELD_ID \
...
for i in $(seq 0 $(($USER_STORY_COUNT-1))); do
  ...
  if [ ! -z "$PARENT_EPIC" ]; then
    ...
    gh projects item-edit $PROJECT_NUM --user '@me' --id $ITEM_ID --field-id $EPIC_LINK_FIELD_ID \
  ...
  if [ ! -z "$POINTS" ]; then
    gh projects item-edit $PROJECT_NUM --user '@me' --id $ITEM_ID --field-id $POINTS_FIELD_ID \
